const Notifier = (() => {
  ...
  const start = async (intervalMinutes) => {
    ...
    // Save prefs
    Storage.setReminderPrefs({ enabled: true, interval: intervalMinutes });
  ...
  const stop = () => {
    ...
    stopSW();
    const current = Storage.getReminderPrefs();
    Storage.setReminderPrefs({ enabled: false, interval: current.interval || 60 });
  };
